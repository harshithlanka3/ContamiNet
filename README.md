...
- **If `CONTAMINET_VLM_PROVIDER=google`**  
  ...
  - Optional: set **`GEMINI_MODEL`** (default `gemini-2.0-flash`; try `gemini-1.5-flash` if your key/project does not expose a model).
...
7. The crop (or full image) is sent to the configured VLM:
   ...
   - **Google**: Gemini `generate_content` with `response_mime_type="application/json"` and `GEMINI_MODEL` (default `gemini-2.0-flash`). The image is sent with MIME type `image/jpeg` for YOLO crops, or the upload’s `Content-Type` when the full image is used.
...
If the model returns non-JSON text, the handler may return `error` and `raw_content` (and still attach `vlm_provider` / `crop_source` / `yolo` when the payload is a dict). Malformed image bytes yield **400**. VLM/network errors yield **500** with a `detail` string.
...
| `GEMINI_MODEL` | `gemini-2.0-flash` | Gemini model id for Google mode. |
